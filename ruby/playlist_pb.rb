# frozen_string_literal: true
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: playlist.proto

require 'google/protobuf'

require 'action_pb'
require 'color_pb'
require 'cue_pb'
require 'customOptions_pb'
require 'hotKey_pb'
require 'musicKeyScale_pb'
require 'planningCenter_pb'
require 'url_pb'
require 'uuid_pb'


descriptor_data = "\n\x0eplaylist.proto\x12\x07rv.data\x1a\x0c\x61\x63tion.proto\x1a\x0b\x63olor.proto\x1a\tcue.proto\x1a\x13\x63ustomOptions.proto\x1a\x0chotKey.proto\x1a\x13musicKeyScale.proto\x1a\x14planningCenter.proto\x1a\turl.proto\x1a\nuuid.proto\"\x91\n\n\x08Playlist\x12\x34\n\tplaylists\x18\x0c \x01(\x0b\x32\x1f.rv.data.Playlist.PlaylistArrayH\x00\x12\x30\n\x05items\x18\r \x01(\x0b\x32\x1f.rv.data.Playlist.PlaylistItemsH\x00\x12<\n\x0fsmart_directory\x18\x0e \x01(\x0b\x32!.rv.data.Playlist.FolderDirectoryH\x01\x12/\n\x08pco_plan\x18\x0f \x01(\x0b\x32\x1b.rv.data.PlanningCenterPlanH\x01\x12\x1b\n\x04uuid\x18\x01 \x01(\x0b\x32\r.rv.data.UUID\x12\x0c\n\x04name\x18\x02 \x01(\t\x12$\n\x04type\x18\x03 \x01(\x0e\x32\x16.rv.data.Playlist.Type\x12\x10\n\x08\x65xpanded\x18\x04 \x01(\x08\x12*\n\x13targeted_layer_uuid\x18\x05 \x01(\x0b\x32\r.rv.data.UUID\x12*\n\x14smart_directory_path\x18\x06 \x01(\x0b\x32\x0c.rv.data.URL\x12 \n\x07hot_key\x18\x07 \x01(\x0b\x32\x0f.rv.data.HotKey\x12\x1a\n\x04\x63ues\x18\x08 \x03(\x0b\x32\x0c.rv.data.Cue\x12#\n\x08\x63hildren\x18\t \x03(\x0b\x32\x11.rv.data.Playlist\x12\x18\n\x10timecode_enabled\x18\n \x01(\x08\x12,\n\x06timing\x18\x0b \x01(\x0e\x32\x1c.rv.data.Playlist.TimingType\x12\x33\n\x0cstartup_info\x18\x10 \x01(\x0b\x32\x1d.rv.data.Playlist.StartupInfo\x1a\x35\n\rPlaylistArray\x12$\n\tplaylists\x18\x01 \x03(\x0b\x32\x11.rv.data.Playlist\x1a\x35\n\rPlaylistItems\x12$\n\x05items\x18\x01 \x03(\x0b\x32\x15.rv.data.PlaylistItem\x1a\xd5\x01\n\x0f\x46olderDirectory\x12%\n\x0fsmart_directory\x18\x01 \x01(\x0b\x32\x0c.rv.data.URL\x12I\n\x0fimport_behavior\x18\x02 \x01(\x0e\x32\x30.rv.data.Playlist.FolderDirectory.ImportBehavior\"P\n\x0eImportBehavior\x12\x1e\n\x1aIMPORT_BEHAVIOR_BACKGROUND\x10\x00\x12\x1e\n\x1aIMPORT_BEHAVIOR_FOREGROUND\x10\x01\x1aO\n\x03Tag\x12\x1d\n\x05\x63olor\x18\x01 \x01(\x0b\x32\x0e.rv.data.Color\x12\x0c\n\x04name\x18\x02 \x01(\t\x12\x1b\n\x04uuid\x18\x03 \x01(\x0b\x32\r.rv.data.UUID\x1a)\n\x0bStartupInfo\x12\x1a\n\x12trigger_on_startup\x18\x01 \x01(\x08\"Z\n\x04Type\x12\x10\n\x0cTYPE_UNKNOWN\x10\x00\x12\x11\n\rTYPE_PLAYLIST\x10\x01\x12\x0e\n\nTYPE_GROUP\x10\x02\x12\x0e\n\nTYPE_SMART\x10\x03\x12\r\n\tTYPE_ROOT\x10\x04\"Y\n\nTimingType\x12\x14\n\x10TIMING_TYPE_NONE\x10\x00\x12\x18\n\x14TIMING_TYPE_TIMECODE\x10\x01\x12\x1b\n\x17TIMING_TYPE_TIME_OF_DAY\x10\x02\x42\x0e\n\x0c\x43hildrenTypeB\n\n\x08LinkData\"\xd6\x06\n\x0cPlaylistItem\x12.\n\x06header\x18\x03 \x01(\x0b\x32\x1c.rv.data.PlaylistItem.HeaderH\x00\x12:\n\x0cpresentation\x18\x04 \x01(\x0b\x32\".rv.data.PlaylistItem.PresentationH\x00\x12\x1b\n\x03\x63ue\x18\x05 \x01(\x0b\x32\x0c.rv.data.CueH\x00\x12?\n\x0fplanning_center\x18\x06 \x01(\x0b\x32$.rv.data.PlaylistItem.PlanningCenterH\x00\x12\x38\n\x0bplaceholder\x18\x08 \x01(\x0b\x32!.rv.data.PlaylistItem.PlaceholderH\x00\x12\x1b\n\x04uuid\x18\x01 \x01(\x0b\x32\r.rv.data.UUID\x12\x0c\n\x04name\x18\x02 \x01(\t\x12\x1b\n\x04tags\x18\x07 \x03(\x0b\x32\r.rv.data.UUID\x12\x11\n\tis_hidden\x18\t \x01(\x08\x1aI\n\x06Header\x12\x1d\n\x05\x63olor\x18\x01 \x01(\x0b\x32\x0e.rv.data.Color\x12 \n\x07\x61\x63tions\x18\x02 \x03(\x0b\x32\x0f.rv.data.Action\x1a\xe2\x01\n\x0cPresentation\x12#\n\rdocument_path\x18\x01 \x01(\x0b\x32\x0c.rv.data.URL\x12\"\n\x0b\x61rrangement\x18\x02 \x01(\x0b\x32\r.rv.data.UUID\x12?\n\x13\x63ontent_destination\x18\x03 \x01(\x0e\x32\".rv.data.Action.ContentDestination\x12.\n\x0euser_music_key\x18\x04 \x01(\x0b\x32\x16.rv.data.MusicKeyScale\x12\x18\n\x10\x61rrangement_name\x18\x05 \x01(\t\x1ap\n\x0ePlanningCenter\x12\x32\n\x04item\x18\x01 \x01(\x0b\x32$.rv.data.PlanningCenterPlan.PlanItem\x12*\n\x0blinked_data\x18\x02 \x01(\x0b\x32\x15.rv.data.PlaylistItem\x1a\x39\n\x0bPlaceholder\x12*\n\x0blinked_data\x18\x01 \x01(\x0b\x32\x15.rv.data.PlaylistItemB\n\n\x08ItemTypeb\x06proto3"

pool = ::Google::Protobuf::DescriptorPool.generated_pool
pool.add_serialized_file(descriptor_data)

module Rv
  module Data
    Playlist = ::Google::Protobuf::DescriptorPool.generated_pool.lookup("rv.data.Playlist").msgclass
    Playlist::PlaylistArray = ::Google::Protobuf::DescriptorPool.generated_pool.lookup("rv.data.Playlist.PlaylistArray").msgclass
    Playlist::PlaylistItems = ::Google::Protobuf::DescriptorPool.generated_pool.lookup("rv.data.Playlist.PlaylistItems").msgclass
    Playlist::FolderDirectory = ::Google::Protobuf::DescriptorPool.generated_pool.lookup("rv.data.Playlist.FolderDirectory").msgclass
    Playlist::FolderDirectory::ImportBehavior = ::Google::Protobuf::DescriptorPool.generated_pool.lookup("rv.data.Playlist.FolderDirectory.ImportBehavior").enummodule
    Playlist::Tag = ::Google::Protobuf::DescriptorPool.generated_pool.lookup("rv.data.Playlist.Tag").msgclass
    Playlist::StartupInfo = ::Google::Protobuf::DescriptorPool.generated_pool.lookup("rv.data.Playlist.StartupInfo").msgclass
    Playlist::Type = ::Google::Protobuf::DescriptorPool.generated_pool.lookup("rv.data.Playlist.Type").enummodule
    Playlist::TimingType = ::Google::Protobuf::DescriptorPool.generated_pool.lookup("rv.data.Playlist.TimingType").enummodule
    PlaylistItem = ::Google::Protobuf::DescriptorPool.generated_pool.lookup("rv.data.PlaylistItem").msgclass
    PlaylistItem::Header = ::Google::Protobuf::DescriptorPool.generated_pool.lookup("rv.data.PlaylistItem.Header").msgclass
    PlaylistItem::Presentation = ::Google::Protobuf::DescriptorPool.generated_pool.lookup("rv.data.PlaylistItem.Presentation").msgclass
    PlaylistItem::PlanningCenter = ::Google::Protobuf::DescriptorPool.generated_pool.lookup("rv.data.PlaylistItem.PlanningCenter").msgclass
    PlaylistItem::Placeholder = ::Google::Protobuf::DescriptorPool.generated_pool.lookup("rv.data.PlaylistItem.Placeholder").msgclass
  end
end
